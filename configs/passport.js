...
module.exports = function (passport) {
  passport.use(
    new LocalStrategy({ membernameField: "membername" }, async (membername, password, done) => {
      try {
        const user = await Members.findOne({ membername: membername });

        if (!user) {
          return done(null, false, { error_msg: "membername does not exist!" });
        }
